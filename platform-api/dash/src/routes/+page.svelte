<script lang="ts">
  ...
  type Tab = "account" | "field" | "ops" | "analytics";
  type ConfirmState = {
    title: string;
    message: string;
    confirmLabel?: string;
  ...
  async function loadAnalytics() {
    ...
    const { res, data } = await api<{
      ...
      days?: GameAnalyticsRow[];
      error?: string;
    }>(`/v1/analytics/games?days=${analyticsDays}`);
    if (!res.ok) {
      analyticsRows = [];
      flash(data.error || "無法載入玩具分析", "err");
      return;
    }
  ...
  function selectTab(next: Tab) {
    if ((next === "ops" || next === "analytics") && !isAdmin) return;
    tab = next;
      ...
      <div class="tabs" role="tablist" aria-label="後台區塊">
        ...
        {#if isAdmin}
          ...
          <button
            ...
            onclick={() => selectTab("analytics")}>玩具分析</button
          >
        {/if}
      </div>
            ...
            <div class="row wrap">
              <div class="seg" role="group" aria-label="listed 篩選">
                ...
                <button
                  ...
                  onclick={() => (analyticsFilter = "unlisted")}
                  >unlisted</button
                >
              </div>
              <label class="meta">
              ...
              <button
                type="button"
                class="secondary"
                onclick={() => void loadAnalytics()}>重新整理</button
              >
            </div>
          </div>
            ...
            {:else}
              <div class="table-wrap">
                <table class="analtable">
                  ...
                  <tbody>
                    {#each analyticsRows.filter(r => analyticsFilter === "all" || (analyticsFilter === "listed") === r.listed) as row (row.day + row.catalog_id)}
                      <tr>
                        <td>
