/**
 * Roster ↔ session invite payloads over avatar_relay (DEC-045 Phase 3 slice 1).
 * Handshake only — no act／event tunnel yet.
 */
...
function parseProtocol(raw: unknown): RosterSessionProtocolSpec | null {
  ...
  const roles = Array.isArray(raw.roles)
    ? raw.roles.filter((r): r is string => typeof r === "string" && r.trim())
    : [];
