<script lang="ts">
  ...
  import {
    HOST_PYTHON_ALLOWED_PACKAGES,
    PYODIDE_VERSION,
    cancelHostPythonRepl,
    ensureHostPython,
    formatReplBanner,
    isPythonScriptPath,
    replHostPython,
  ...
  import type { ReplPrompt } from "./hostPythonRepl";
  ...
  interface Props {
    /** Current work-project files (OPFS snapshot in shell memory). */
    files?: FileMap;
    projectId?: string | null;
    disabled?: boolean;
    visible?: boolean;
    onStatus?: (detail: string) => void;
    onPathsChanged?: (paths: { changed: string[] }) => void | Promise<void>;
  }
  ...
  let lineBuf = "";
  let prompt: ReplPrompt = REPL_PRIMARY_PROMPT;
  let history: string[] = [];
  let historyIndex = -1;
  let booted = false;

  function collectProjectPyFiles(): Record<string, string> {
    const out: Record<string, string> = {};
    for (const [raw, content] of Object.entries(files)) {
      try {
        const path = normalizeProjectPath(raw);
        if (!isPythonScriptPath(path)) continue;
  ...
  async function boot() {
    ...
    busy = true;
    setDetail("載入 Pyodide…");
    try {
      const { pyodideVersion } = await ensureHostPython();
      ready = true;
      booted = true;
      term?.reset();
      for (const line of formatReplBanner(
      ...
      lineBuf = "";
      writePrompt();
      setDetail(`就緒 · Pyodide ${pyodideVersion || PYODIDE_VERSION}`);
      term?.focus();
    } catch (e) {
  ...
  async function submitLine(raw: string) {
    ...
    busy = true;
    try {
      const result = await replHostPython(raw, {
        projectId: projectId ?? undefined,
        projectFiles: collectProjectPyFiles(),
      });
      ...
      if (result.changedPaths?.length) {
        await onPathsChanged?.({ changed: result.changedPaths });
      }
  ...
  async function handleReset() {
    ...
    busy = true;
    try {
      await resetHostPython();
      prompt = REPL_PRIMARY_PROMPT;
      lineBuf = "";
      term?.writeln("");
      term?.writeln("[playgrounds] 已重設解譯器（套件仍保留）");
      writePrompt();
      setDetail("已重設");
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
...
<div
  class="playgrounds-python-repl bg-skin-fill text-skin-base flex h-full min-h-0 flex-col"
>
  <div
    class="border-skin-line flex h-8 shrink-0 items-center gap-2 border-b px-2.5 text-[11px]"
  >
    <span class="text-skin-base/50">Python</span>
    ...
    <div class="ml-auto flex flex-wrap items-center gap-1">
      ...
      <button
        ...
        class="playgrounds-repl-btn"
        disabled={busy || disabled || !ready}
        onclick={() => void handleReset()}>重設解譯器</button
      >
    </div>
  </div>
  <div bind:this={hostEl} class="playgrounds-python-repl-xterm min-h-0 flex-1 px-1 py-1"></div>
</div>
...
<style>
  ...
  .playgrounds-python-repl-xterm :global(.xterm) {
    height: 100%;
  }
  .playgrounds-python-repl-xterm :global(.xterm-viewport) {
