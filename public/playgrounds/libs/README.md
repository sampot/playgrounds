...
- **Split ESM builds:** sibling chunks an ESM entry imports (e.g. three → `three.core.min.js`) must be vendored **and** listed in that pin's `assets`. A missing chunk resolves to the SPA fallback (200 `text/html`), so the failure looks like a MIME error, not a 404.
...
| three | `three-0.185.1.module.min.js` (+ `three.core.min.js`) | other | ESM namespace | MIT |
