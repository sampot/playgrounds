...
| `env.HOST`（DEC-017／051） | **對口席（總管）**＝動態全量 API 客戶端（全目錄 scopes 自動準入快捷）。**已準入 scopes 的任意 SAM**＝**同形子集**（`HOST.capabilities()`／可呼叫面＝投影；另見 `listKnownScopes`／`listAdmittedScopes`）。跨沙盒 FS 另需 grant（明示／建立即自動；`grantSandboxAccess` 對口可用）。**不**另立 `env.SANDBOX`／`env.OBSERVE`。見 [PG-API-SCOPES-SPEC.md](./PG-API-SCOPES-SPEC.md)。 |
