...
| `env.COMPUTE`（DEC-036／051） | 一般 SAM 經 `sam:capabilities` 宣告＋使用者準入後注入的**窄** compute binding（`compute:python`／`compute:cmd`；舊名 `runPython`／`runCmd`）；≠ 完整 HOST。自己的檔案／`vars`／KV／DB 為 intrinsic、不必宣告。Scopes 目錄見 [PG-API-SCOPES-SPEC.md](./PG-API-SCOPES-SPEC.md)。 |
