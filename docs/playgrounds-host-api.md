...
給撰寫／修改「現行 Agent」這類單頁小程式（SAM）的人（含 Agent 自己）快速對照。沙盒主軸仍是輕量 Web；Agent 用途可不限 coding。HOST 僅注入給現行 Agent（對口席全量客戶端）。權威決策見 [DECISIONS.md](./DECISIONS.md) DEC-016／017／018／019／020／021／022／023／**024**／**029**／**030**／**031**／**032**／**035**／**036**／**044**／**051**（Proposed）／**053**（UI 只走 `/api` → `functions.js`；shell/runtime 走 env binding）；階段見 [PG-AGENT-PLAN.md](./PG-AGENT-PLAN.md)、[PG-SAM-RUNTIME-PLAN.md](./PG-SAM-RUNTIME-PLAN.md)、[PG-SECRETSTORE-PLAN.md](./PG-SECRETSTORE-PLAN.md)、[PG-SAM-ENV-SPEC.md](./PG-SAM-ENV-SPEC.md)、[PG-API-SCOPES-SPEC.md](./PG-API-SCOPES-SPEC.md)、[PG-MAIN-CONTENT-PLAN.md](./PG-MAIN-CONTENT-PLAN.md)、[PG-BOTTOM-DOCK-PLAN.md](./PG-BOTTOM-DOCK-PLAN.md)、[PG-AGENT-MODEL-PLAN.md](./PG-AGENT-MODEL-PLAN.md)、[PG-AGENT-FLEET-UX-PLAN.md](./PG-AGENT-FLEET-UX-PLAN.md)。
...
| `env.secrets.*`（DEC-029／035／051） | **每 secret 一顆**：`env.secrets.<NAME>`＝`{ get(): Promise<string> }`；須準入 **`secrets:get`**（對口自動具備）且 unlock。值永不進 `.sam`；HOST 不回傳值。`secrets:list` 僅 HOST 列 meta／名稱。Tool 為空命名空間。**無** `env.SECRETS`／bag／頂層密鑰鍵。 |
| `env.HOST`（DEC-017／051／053） | **對口席（總管）**＝動態全量 API 客戶端（全目錄 scopes 自動準入快捷）。**已準入 scopes 的任意 SAM**＝**同形子集**（`HOST.capabilities()`／可呼叫面＝投影；另見 `listKnownScopes`／`listAdmittedScopes`）。跨沙盒 FS 另需 grant（明示／建立即自動；`grantSandboxAccess` 對口可用）。**不**另立 `env.SANDBOX`／`env.OBSERVE`。**go 純玩版同形注入：** [`createGoHostBinding`](../../go-client/src/lib/goHostBinding.ts) factory 在 hostable sandbox 把 `hostRuntime`（`env.KV` authority）＋`goAuth`（Platform invite 代理）包成同形 `HostBridge`；與場殼 `play` 共用同 binding 名，SAM 同一份 `functions.js` 可攜。詳見 [PG-GO-CLIENT-PLAN.md §6.7.1](./PG-GO-CLIENT-PLAN.md)。 |
| `env.COMPUTE`（DEC-036／051） | 遷移雙掛：已準入 `compute:python`／`compute:cmd`（舊名 `runPython`／`runCmd`）時注入；方法亦出現在 HOST 子集。自己的檔案／`vars`／KV／DB 為 intrinsic、不必宣告。Scopes 目錄見 [PG-API-SCOPES-SPEC.md](./PG-API-SCOPES-SPEC.md)。 |
