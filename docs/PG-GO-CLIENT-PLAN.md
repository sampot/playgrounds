...
| **6e. 包廂（`/room`）** | 已登入進 `/room` **即包廂 UI**；Guest `/i/` 進同一間且**留在 `/i/`**；**入座不鎖 1:1**；**目錄只掛檔**；**live 在名單**；影音檔＝漸進下載播放；主視訊區（片子／live RTP；劇院態滿窗＋overlay；開局延後）；**現況 Hub**；包廂活著＝主持畫面開著 | 見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md) | 按需鑄／兩個時鐘／2c 電視已落地；**2d 殼面進行中**（廳態＋劇院態已接）；**3 開局延後** |
...
| 包廂（`/room`；臨時隔間；**活著＝主持畫面開著**；主面＝主視訊區；劇院態滿窗＋overlay；門牌仍 `/i/`、TTL 只管請新人；開口為主、文字為輔；兩層螢幕；開局延後；現況 Hub 轉送、mesh 延後） | 聊天室、房間、視訊會議格子牆、把局內 overlay 叫包廂；`/chat` 當 canonical；5 分鐘租期的雲端房間；再開 `/room` 當連上既有這一間；可行走大廳複製；用系統全螢幕冒充劇院態 |
...
| 2026-08-19 | 包廂：**電視主面**（節目 RTP、主持導播、兩層螢幕、文字為輔、靜態內景）。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md) |
