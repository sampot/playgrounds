...
> **相關：** DEC-004（對外敘事）、DEC-009／016（SW／路徑）、DEC-025（`?open=`）、DEC-024（`sam-runtime`）、DEC-040（整場重置／origin 資料）、DEC-043／[PG-DOCS-PLAN.md](./PG-DOCS-PLAN.md)（文件站 `docs.samkuo.me`）、DEC-048／[PG-SVELTEKIT-PLAN.md](./PG-SVELTEKIT-PLAN.md)（宿主建置→SvelteKit Draft）、[PG-CATALOG-UX-PLAN.md](./PG-CATALOG-UX-PLAN.md)（型錄 UX）
