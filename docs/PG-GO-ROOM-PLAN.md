...
| --- | --- | --- | --- |
| **廳態** | 關機、進門預設；未登入／connecting／ended **只廳態** | 最大可用的 16:9（直式在上；短橫屏左；寬屏可與欄並排）。槽內無字 | 可在文檔流：窄屏 tab、寬屏欄。底列麥／鏡頭／畫面。廣告浮在主視訊區（關機時） |
...
| 模式 | 條件（約） | 廳態結構 |
| --- | --- | --- |
| **手機直式** | 預設 | 主視訊全寬 16:9 在上；底下**同一面板**三段切換：成員／檔案／文字（預設**成員**）。底列只放麥／鏡頭／畫面 |
| **手機橫式** | `landscape` 且短高（現況 ~560px） | 左電視幾乎滿高、右同一分段面板；廣告浮在電視槽（關機時；串流藏）；頂緣 ≥44px 隱形熱區拉出殼。短橫屏高度不夠時**可直接劇院態** |
| **平板** | `min-width: 40rem` 且高度夠 | 電視仍最大；成員＋檔案＋文字**可同時露出**。直式三欄在電視下；橫式可與桌機同一套「電視左、三區右疊」 |
| **桌機** | `min-width: 64rem` | 電視左約 2／3；右欄約 20–24rem 上成員、中檔案、下文字。門牌小狀態在成員區 |
...
2. **一句狀態**  
   人數＋電視（關機／正在播〈名〉／電視上是〈誰〉／正在玩〈遊戲〉）。**不講 TTL。**  
...
4. **分段面板**  
   **成員**（預設）／**檔案**／**文字**。廳態＝電視下方 tab（電視始終可見）。劇院態＝同一套 **drawer overlay**（可關；主視訊仍在底下）。
5. **底列**  
   開麥克風（主）／鏡頭／畫面。劇院態＝薄 overlay，可跟頂列一起收。快捷語在文字區、預設收起。主持「放到電視上」「玩遊戲」在成員／檔案／電視控制，不擠底列。
6. **文字**  
...
**廳態：** 電視左（可較高）。右欄同時露出成員、檔案、文字（平板直式可改電視下三欄）。規則同 §5.8。
...
| 28 | 殼面 | **主視訊區**＝主內容（16:9；關機也佔；**槽內無字**）。廳態：成員／檔案／文字在流裡（直式 tab、橫式左電視右欄、平板可三區、桌機右欄）。**劇院態：** 應用內滿窗；三區／底列 overlay／drawer。廣告浮在主視訊區，**串流時藏**。**禁止**以系統 Fullscreen 當劇院態預設。**不**用可行走大廳。家具熱點非主導航 |
