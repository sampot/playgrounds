...
> **狀態：** Draft（2026-08-19）— 主面＝**主視訊區**（關機／片子／live 同一塊；槽內無字；**劇院態可滿窗**，三區／底列 overlay；**開局契約仍凍、實作延後**）；chrome **可收**；廳態＝電視＋**成員／檔案／文字**（RWD）；文字＝開口備援（非主欄）；兩層螢幕（電視 ≠ 我這台）；目錄檔私下播／下載走 DC；其餘：進門即主面、不鎖 1:1、兩個時鐘、Hub 星狀、SDP **2+2**
...
- **進門即主面（硬）：** 已登入會員開 `/room`＝已經在包廂裡。**主面是包廂電視槽**（關機／片子／live／開局都佔主高度）。邀請是**面內動作**（請人進來），**不是**進門條件。一個人在也是這一間。**禁止**用全頁時間線當進門第一眼。
...
- **電視＝主持導播（硬）：** 這一間同時只有一個電視來源（關機／檔／某 peer 的 live／**這一局遊戲**）。來源由主持指定。看電影＝主持決定廣播哪一步（來源端播放器是時鐘）；切 live＝現在電視上是誰；開局＝選哪一款、哪些人入座（其餘觀戰）。被指定上電視或入座的人不是新主持。見 §5.7、§5.9。
...
11. **主面＝主視訊區；分享面＝邀請** — 主面回答「電視上是什麼／誰在這一間」。QR／TTL／再發一張只出現在「請人進來」分享面（寬屏成員區最多門牌**小狀態**，不常駐大 QR）。**禁止**把時間線當主欄。殼頂列對齊對弈：**可 overlay 收起**。看電影／live／（日後）開局走**劇院態**：主視訊滿窗，三區與底列改 overlay／drawer（§5.8）。
...
16. **內容按需拉；房級電視例外（硬）** — 目錄檔 bytes、在場**影像**：**禁止**在沒有該 peer 的 `request` 時送。Host 只轉給請求者。不做格子牆。每人最多一條出站在場 live。私下播檔走 DC，不佔節目槽。**房級：** 在場自動收**節目**（電視）與（開麥者的）**在場聲**——進門即對節目／在場音 `request`；**不**自動 `request` 在場視訊、**不**自動開相機。收看端 **PC 一建立就綁節目 `<video>`**（電視槽）；關機／開局時可藏視覺、**不要** `display:none` 以免解碼停。人數不是鏡頭開關。
...
| 用 | 不用 |
| --- | --- |
| 包廂、進包廂、請人進來、**結束這一間**（主持）、**離開這一間**（Guest）、再發一張邀請、分享區、分享目錄、掛上、下載、檢視、收看、收聽、撤回、**包廂電視**、**放到電視上**、**私下播放**、**玩遊戲**、**結束這一局**、成員、檔案、文字 | 聊天區、聊天室、房間、Room、Lounge、Lobby（對讀者）；「先邀請才能進包廂」；附加檔、傳給對方、接收附件；Guest 說「結束這一間」；把時間線叫主畫面；把包廂開局叫「邀請對弈」（那是 GO-INVITE） |
...
| 區 | 角色 |
| --- | --- |
| **電視／主視訊區** | 主舞台。同一塊 slot：關機也佔位；片子／live＝節目 `<video>`（進門即綁，**不要** `display:none`）；開局＝同一塊掛 SAM 畫布。預設 **16:9**、`object-fit: contain`。**槽內禁止疊字**（片名、關機、人數都在槽外狀態或 overlay）。**不要**再把大廳 320×200 內景當頁面主內容、把節目縮成牆上小洞。 |
...
pixel 畫風可留在**電視外框／關機雪花**；**禁止**行走、碰撞、重用 `GoShopLobby` 地圖。門／架／椅**不是**主導航（請人／檔案／成員走三區）。
...
**廣告版位：** DOM 同一 `GoAdSlot`，**浮在主視訊區內**（關機／空槽時）。**節目串流（片子／live／日後開局）自動藏**——劇院態因此也不露。不佔流高度、不另掛 sticky 搶電視。點進 `/s/` 若包廂仍 live → **頁內確認**（開局中須先結束這一局或一併說明散場）。遊戲 Invite 同意面與 **GO-INVITE 對弈中**仍不掛（見 [PG-GO-ADS-PLAN.md](./PG-GO-ADS-PLAN.md)）。內景後牆看板若還畫，純裝飾、不進熱點。浮動廣告是標籤版位，**不是**槽內節目字幕／關機文案。

**兩種殼態（硬）：**

| 態 | 何時 | 主視訊 | 成員／檔案／文字、底列、廣告 |
| --- | --- | --- | --- |
| **廳態** | 關機、進門預設；未登入／connecting／ended **只廳態** | 直式上半；短橫屏左；寬屏可與欄並排。影片 `object-fit: contain`。槽內無字 | 可在文檔流：窄屏下半 tab、寬屏右欄。麥／鏡頭列在控制面板上方。廣告浮在主視訊區（關機時） |
| **劇院態** | 片子／live 亮著（可自動進或「展開」）；日後開局同一套 | **應用內滿窗**（視窗客戶區／`100svh`；同一顆進門綁定的 `<video>` 或日後畫布；`object-fit: contain`）。關機視覺可留在滿窗裡 | **全部 overlay／drawer**，不佔流高度。頂列維持 overlay 可收。廣告因串流而藏。drawer 打開時電視仍在底下，**不要** `display:none` |

- **禁止**用瀏覽器 Fullscreen API（含 iOS `<video>` 全螢幕）當劇院態的**唯一**或**預設**手段。系統全螢幕裡殼 overlay／麥／成員不可用；開局畫布必須留在文件。系統全螢幕可當加分，從劇院態再點。
- 關機 → 回廳態。Esc／關 drawer：**先關 overlay**；不要第一次 Esc 就散場。無 overlay 時 Esc 可縮回廳態（仍在包廂）或沿用既有散場確認——**劇院態本身不是散場**。
- 分享面、結束確認、composer focus：暫停頂列自動收；drawer 開著時同。
- 讀屏：滿窗仍是那顆 `<video>`／可及遊戲面；三區改 drawer 後仍要有明確開關（成員／檔案／文字全名；熱區 ≥44px）。

**RWD（mobile-first；禁止桌面先做再 `max-width` 縮小）：** 預設 CSS＝手機直式。可用容器查詢掛在包廂根（殼有側欄時勿誤判已是桌機）。下列是**廳態**；劇院態一律滿窗＋overlay，不再依斷點把三區排進流裡。

| 模式 | 條件（約） | 廳態結構 |
| --- | --- | --- |
| **窄屏** | 寬不到 768px | 螢幕對半：上半主視訊（槽內無字）；**下半控制面板** tab 切換成員／檔案／文字（預設**成員**）。麥／鏡頭／畫面列在控制面板**上方**。**禁止**三欄並排 |
| **短橫屏** | `landscape` 且短高（現況 ~560px；含寬過 768 的手機橫式） | 左電視、**右控制面板三段 tab**（成員／檔案／文字，一次一區，**不是**上檔案下成員）。麥列在右欄頂。廣告浮在電視槽（關機時；串流藏） |
...
廳態窄屏三段是 **tab**，不是蓋滿螢幕、把電視藏起來的 modal。劇院態同一套三段改成 **drawer**（蓋在主視訊上，可關）。熱區 ≥44px；分段標籤寫全名。

**頂列可收（硬）：** 對齊對弈 chrome（overlay、閒置約 3s、往下拉／頂緣展開、收起**不留**角標 logo）。進包廂**主面**即 hideable（主持 `/room`、Guest 同意後的 `/i/`）。consent／未登入／connecting／ended：**不收**。分享面、結束／離開確認、更多、profile、登入 sheet、窄屏正在填文字輸入、劇院態 drawer 開著時：**暫停收起**。請人進來與結束／離開**不得只活在頂列**——請人在成員區（廳態常駐；劇院態在成員 drawer）。結束／離開在底列或成員區底（劇院態＝底列 overlay）。人數／電視一句在**槽外**（廳態電視下緣薄狀態；劇院態可疊視窗邊緣 overlay），**不要**寫進主視訊畫面裡。
...
| `session_cast` | 媒體階段 | **電視控制面**（指定片子／live 來源／關機；可選 name／paused／t 當標籤）；**不**承載影音 bytes；節目 RTP 走 Hub |
...
| **產品面** | 電視槽；關機也是主視覺。開局時同一塊改掛 SAM，不是第二個舞台 |
| **傳輸** | WebRTC **節目** RTP。現況經 Host 轉給每位在場。**禁止**用 DC 當電視 |

**電視綁定（硬）：** 進門 PC 一建立，就把遠端 **program** receiver 綁到電視槽 `<video srcObject>`。禁止等有畫面才第一次綁。關機＝空軌或關機視覺；開局可藏視覺改掛畫布，**不要**拆綁定、**不要** `display:none`。
...
**主面是主視訊區**（關機也是主體；槽內無字）。看電影／開局走劇院態滿窗。分享面才回答「這張邀請還能不能用」。文字不是 UI 主體。不要一進門就是每人一格鏡頭。殼面細節（廳態／劇院態、RWD、頂列可收）見 **§5.8**；開局見 **§5.9**。
...
2. **一句狀態**  
   人數＋電視（關機／正在播〈名〉／電視上是〈誰〉／正在玩〈遊戲〉）。**不講 TTL。** 一個人且電視關機：**不要**寫 `就你一個人 · 把這頁開著，這一間才還在`（這行拿掉）。有人：`3 人在`。門牌過期**不**取代這一行。
3. **主視訊區（主體）**  
   廳態：全寬 16:9。劇院態：滿窗。關機＝雪花／黑屏、**不要在畫面裡寫字**。片子／live＝DOM `<video>`。開局＝同一塊 SAM 畫布。**不要**把整間 pixel 內景當主內容。
4. **分段面板**  
   **成員**（預設）／**檔案**／**文字**。廳態直式（不到 768px）＝螢幕下半 tab（上半電視始終可見）。劇院態＝同一套 **drawer overlay**（可關；主視訊仍在底下）。
5. **麥／鏡頭列**  
...
**手機橫式：** 廳態左電視、右分段；廣告浮在電視槽（關機時）；頂緣拉出殼。高度不夠可直接劇院態。見 §5.8。

### 10.3 寬屏（`min-width` 遞增）

**廳態：** 電視左（可較高）。**右欄控制面板佔滿視窗高**：頂列麥／鏡頭，其下上半檔案、下半成員／文字 tab。**禁止**三欄並排。規則同 §5.8。

**劇院態（片子／live／日後開局）：** 主視訊滿窗；右欄改成 overlay drawer（可同時疊或一次一區）。不要為了露出三欄而把電影縮小。
...
Esc 回大廳（現況 `goEscapeHome` 含 `/chat` → 改 `/room`）。**劇院態／drawer／分享面／確認／文字焦點：Esc 先關 overlay**，不要第一次就散場。無 overlay 且已廳態才走散場確認。並**暫停**頂列自動收。

### 10.5 電視、私下播放、開局（契約現在凍）

- 主視訊區永遠在舞台；關機也在。槽內**無字**。節目 `<video>` 綁定進門即做；開局改掛畫布時可藏視覺，**不要**拆綁、**不要** `display:none` 到解碼停（切回片子時還要用）。
...
- 點主視訊（非開局）可展開完整控制（音量／系統全螢幕／主持的播放控制）——控制面是 overlay，不是把字疊回畫面上。
...
| 殼面 | 主視訊區 16:9（槽內無字）＋廳態三區／劇院態 overlay（§5.8）。內景降級為外框／關機美術。**不要**重用 `GoShopLobby`。chrome hideable 對齊對弈 overlay。**不要**用系統全螢幕冒充劇院態 |
...
| **2c. 包廂電視＋內景** | 靜態內景；節目槽＝電視；主持指定來源（檔 `captureStream` 或 peer live）；房級收節目／在場聲；文字收成抽屜 | 關機電視佔主高度；一起看 MTV 走 RTP；私下播與電視並行；不走動 | **已落地**（`GoBoothStage`；節目 RTP；文字／分享抽屜）。殼面改電視槽＋三區見 **2d** |
| **2d. 電視槽殼面** | 主視訊區（關機也佔；槽內無字）；廳態三區 RWD；**劇院態滿窗＋三區／底列 overlay**；頂列 overlay 可收；家具熱點降為非主導航 | 直／橫／平板／桌機可用；看電影時主視訊滿窗、其餘 overlay；請人在成員區／drawer | **進行中**（廳態＋劇院態已接；手測／RWD 收斂中） |
...
| 28 | 殼面 | **主視訊區**＝主內容（16:9；關機也佔；**槽內無字**）。廳態：不到 768px 上半電視下半 tab；≥768px 右欄控制面板（頂麥列、上檔案、下成員／文字 tab）。**禁止**三欄並排。**劇院態：** 應用內滿窗；三區／底列 overlay／drawer。廣告浮在主視訊區，**串流時藏**。**禁止**以系統 Fullscreen 當劇院態預設。**不**用可行走大廳。家具熱點非主導航 |
...
| 2026-08-19 | **殼面＋開局契約：** 電視槽＝主內容（16:9；關機／片子／live／開局同一塊）；成員／檔案／文字三區 RWD；頂列 overlay 可收；開局重用進門 PC（主持選遊戲＋指定／自動入座；不鑄 compose）。凍結 #28–#30 |
...
| 2026-08-19 | **麥列改 icon：** 開麥克風／鏡頭／畫面／關電視／結束改 44px icon（aria-label 全名） |
