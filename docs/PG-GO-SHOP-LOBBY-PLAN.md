...
| **`/room`** | ❌ 不重做大廳行走地圖 | 獨立**靜態內景**＋電視（見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md) §5.8）；進門卸載大廳 canvas |
| **`/s/<id>` 載入／錯誤** | ❌ | 可「回遊樂場」→ `/` |
...
| **包廂** | 臨時隔間 `/room`（壽命＝主持畫面開著；主面＝電視；私下播不互斥；說／掛／要） | 熱點 **包廂門** → `/room` | 局內 overlay；公開聊天區；大廳可行走地圖；視訊會議格子牆 |
