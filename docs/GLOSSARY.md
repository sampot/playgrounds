...
| 包廂遠端操作者（go） | Operator／遠端操作者 | 帳號擁有者的 **Remote Owner Shell**（go `/room/remote`）；與本地 `/room` 同 **Owner** 權限：導播 cast、**Hub 私有片庫讀寫**（Owner file channel；非雲同步）、分享目錄管理；**不是** Guest（不走 `/i/`）。監控切台只是應用情境之一。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md) §6.1。 |
