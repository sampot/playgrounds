...
| 純玩版包廂（go） | 包廂／`/room` | go 殼層**臨時隔間**：壽命＝**主持 `/room` 畫面還開著**。**已登入會員開 `/room` 即包廂主面**（按「請人進來」才鑄門牌）。短鏈 **`/i/<short>`** 請人進來，也可給自己的另一台掃。**入座不鎖 1:1**。**主面＝主視訊區**（沒訊號／片子／live 同一塊；槽內無字；廳態三區在流裡；**劇院態應用內滿窗**，成員／檔案／文字／底列 overlay）。片子／圖／音／live＝節目 RTP（**放到大螢幕上**＝主持指定**一路**；再指定＝切台；**持檔端**本機渲染，可別人掛的檔）；**否決**多路視訊合成進大螢幕。開口為主；星狀下在場聲＝Host 混音（§9.8.1／2f 已落地）。**開局＝契約凍、實作延後**。每人自帶裝置可掛／下載／**私下播**（前端一律 **`/room-file/<id>`**；本機掛檔 SW 直出不經 DC；遠端每 HTTP ↔ `transferId`），**不跟大螢幕互斥**。殼頂列可收。分享目錄只掛檔。**現況經主持 Hub 轉送**（mesh 延後）。資料面只走 WebRTC，**不**雲存、**不錄製**。**不是**視訊會議格子牆、**不是**全頁聊天室。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、DEC-050／045／047。 |
