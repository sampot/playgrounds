...
| 純玩版包廂（go） | 包廂／`/room` | go 殼層**臨時隔間**：現行壽命＝**主持 `/room` 畫面還開著**（**修訂草案**見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)：Engine session／`pg-boothd` 常駐）。**已登入會員開 `/room` 即包廂主面**（按「請人進來」才鑄門牌）。短鏈 **`/i/<short>`** 請人進來，也可給自己的另一台掃。**入座不鎖 1:1**。**主面＝主視訊區**（沒訊號／片子／live 同一塊；槽內無字；廳態三區在流裡；**劇院態應用內滿窗**，成員／檔案／文字／底列 overlay）。片子／圖／音／live＝節目 RTP（**放到大螢幕上**＝主持指定**一路**；再指定＝切台；**持檔端**本機渲染，可別人掛的檔或**主持私有 OPFS**；上大螢幕 ≠ 分享；**音檔＝全場 audio player 面＋節奏動畫，僅主持控時鐘——契約 2h／#34**）；**否決**多路視訊合成進大螢幕。開口為主；星狀下在場聲＝Host 混音（§9.8.1／2f 已落地）。**開局＝契約凍、第一刀手測**（落地見 [PG-GO-ROOM-PLAY-PLAN.md](./PG-GO-ROOM-PLAY-PLAN.md)）。每人自帶裝置可掛／下載／**私下播**（分享目錄前端一律 **`/room-file/<id>`**；本機掛檔 SW 直出不經 DC；遠端每 HTTP ↔ `transferId`），**不跟大螢幕互斥**。**主持私有檔**（OPFS；與分享分離；2g 已落地）可推播大螢幕但不開放「要」。殼頂列可收。分享目錄只掛檔。節目／在場 RTP 經主持 Hub；檔 bytes 可 Guest↔Guest mesh（1c）。資料面只走 WebRTC，**不**雲存；主持可選**多路 live 錄影**至 Hub 私有片庫（`session_record`；見 [PG-GO-ROOM-RECORD-PLAN.md](./PG-GO-ROOM-RECORD-PLAN.md)；**未落地**）。**不是**視訊會議格子牆、**不是**全頁聊天室。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、[PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)、DEC-050／045／047。 |
| 包廂引擎（go） | Booth Engine／包廂引擎 | 分 **Hub**（包廂本體；**唯一**連 Platform）與 **Peer**（headless 終端；只連 Hub 貢獻檔／live）。Hub＝WebRTC Hub、門牌、cast、私有片庫；**無 UI**。Embedded 或外部 **`pg-boothd`**（hub；私有 repo）。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)（**未落地**）。 |
...
| 包廂 Peer 引擎（go） | Booth Peer Engine／Peer 引擎 | headless **來源裝置**（監控鏡頭、邊緣掛檔）；經 Hub 本機 **`peerCap`** 進房；**不**連 Platform。外部 **`pg-boothd peer`**（私有 repo）。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)。 |
| 包廂殼（go） | Booth Shell／包廂殼 | 包廂 **UI 客戶端**：只連 **Hub**、送 intent；**無** Hub 權威。含 go `/room`、遠端 **Operator** 面。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)。 |
| 包廂常駐 daemon（go） | `pg-boothd` | **獨立私有 repo** 的 Rust 二進位（hub／`peer`）；**非**開源；**不在** `playgrounds`。監控／NAS 常駐；瀏覽器 `/room` 為 Embedded Hub 替代。契約見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)。 |
