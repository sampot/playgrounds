...
| 純玩版客戶端（Playgrounds） | 純玩版／`go.samkuo.me` | 獨立 Worker＠**`https://go.samkuo.me`**：**無編輯環境**、不依賴持久 OPFS 沙盒庫；**同時只跑一個 SAM**。啟動含（1）Invite 短鏈入座（**臨時**；**不能離線**）（2）型錄 **`/s/<catalog_id>`** 單機傳閱（3）首頁 `/` 至多 3 則推薦（picks 優先、可跨 kind）。`/s/` 可**同 kind 換片**，並可**加主畫面／造訪後離線／本機分數**（鍵＝`catalog_id`）；`/i/` 不換片、不離線。UI **須**露出山姆鍋 logo、遊樂場主網址 **`play.samkuo.me`**，以及條件允許時的 Header「分享」（開分享面→`/s/<id>`，**不是**邀請短鏈）。**不是**場。見 [PG-GO-CLIENT-PLAN.md](./PG-GO-CLIENT-PLAN.md) §5.5／§6.5、DEC-050。 |
| 型錄 SAM 傳閱連（go） | `/s/<catalog_id>`／go 分享網址 | 只認型錄穩定 **`id`** 的純玩深鏈：**`https://go.samkuo.me/s/<id>`**。go **建置內嵌** catalog 解析；無 Invite／join／TTL。型錄列「分享」與 go Header「分享」只出此形；go Header 開**頁內分享面**（系統分享／**QR**／複製；現場掃碼快樂路徑）；**分享 title／社群 `og:title`＝該筆 `entry.title`**（每 id 不同；首包 HTML 可見）。單機模式下可同 **`kind`**「下一個」／推薦≤3 換片（取代 slot）；可安裝、造訪後離線再玩、本機分數。**不是** `/i/`、**不是**場 `?open=`／`view=canvas`、**不是**「一鍵開」、**不是** go 上完整型錄、**不是**邀請 QR。見 DEC-050、[PG-GO-CLIENT-PLAN.md](./PG-GO-CLIENT-PLAN.md) §5.5／§5.5.1、[PG-CATALOG-UX-PLAN.md](./PG-CATALOG-UX-PLAN.md)。 |
