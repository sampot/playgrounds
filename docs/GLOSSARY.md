...
| 純玩版遊樂場大廳 Lobby（go） | 大廳／lobby | go **首頁 `/`** 的 **室內遊樂場大廳** canvas 殼（湯姆熊式：走道、機台、服務台；**不是** outdoor 全園、**不是** RPG 道具店）：**單一 2D `<canvas>`**；**輕量行走**＋hit-test（老闆／布告欄／詢問處／**機台**／後場／**包廂門**等）；**殼是大廳、小品仍是小品**；清單模式（DOM）永遠可達；`/s/`／`/i/`／`/room` **深鏈 bypass**（包廂是獨立殼面：**大螢幕槽**＋成員／檔案／文字，**不是**大廳可行走地圖；開局延後，屆時才在大螢幕槽掛 SAM）。**禁止** DOM 當大廳地圖本體。見 [PG-GO-SHOP-LOBBY-PLAN.md](./PG-GO-SHOP-LOBBY-PLAN.md)、[PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、DEC-050。 |
| 純玩版包廂（go） | 包廂／`/room` | go 殼層**臨時隔間**：現行壽命＝**主持 `/room` 畫面還開著**（**修訂草案**見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)：Engine session／`pg-boothd` 或 **`pg-booth-desktop`** 常駐）。**已登入會員開 `/room` 即包廂主面**（按「請人進來」才鑄門牌）。短鏈 **`/i/<short>`** 請人進來，也可給自己的另一台掃。**入座不鎖 1:1**。**主面＝主視訊區**（沒訊號／片子／live 同一塊；槽內無字；廳態三區在流裡；**劇院態應用內滿窗**，成員／檔案／文字／底列 overlay）。片子／圖／音／live＝節目 RTP（**放到大螢幕上**＝主持指定**一路**；再指定＝切台；**持檔端**本機渲染，可別人掛的檔或**主持私有 OPFS**；上大螢幕 ≠ 分享；**音檔＝全場 audio player 面＋節奏動畫，僅主持控時鐘——契約 2h／#34**）；**否決**多路視訊合成進大螢幕。開口為主；星狀下在場聲＝Host 混音（§9.8.1／2f 已落地）。**開局＝契約凍、第一刀手測**（落地見 [PG-GO-ROOM-PLAY-PLAN.md](./PG-GO-ROOM-PLAY-PLAN.md)）。每人自帶裝置可掛／下載／**私下播**（分享目錄前端一律 **`/room-file/<id>`**；本機掛檔 SW 直出不經 DC；遠端每 HTTP ↔ `transferId`），**不跟大螢幕互斥**。**主持私有檔**（OPFS；與分享分離；2g 已落地）可推播大螢幕但不開放「要」。殼頂列可收。分享目錄只掛檔。節目／在場 RTP 經主持 Hub；檔 bytes 可 Guest↔Guest mesh（1c）。資料面只走 WebRTC，**不**雲存；主持可選**多路 live 錄影**至 Hub 私有片庫（`session_record`；見 [PG-GO-ROOM-RECORD-PLAN.md](./PG-GO-ROOM-RECORD-PLAN.md)；**未落地**）。**不是**視訊會議格子牆、**不是**全頁聊天室。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、[PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)、[PG-GO-ROOM-TAURI-PLAN.md](./PG-GO-ROOM-TAURI-PLAN.md)、DEC-050／045／047。 |
| 包廂引擎（go） | Booth Engine／包廂引擎 | 分 **Hub**（包廂本體；**唯一**連 Platform）與 **Peer**（headless 終端；只連 Hub 貢獻檔／live）。Hub＝WebRTC Hub、門牌、cast、私有片庫；**無 UI**。Embedded 或私有 monorepo **`pg-booth`** 的 **`pg-boothd`**／**`pg-booth-desktop`**（hub）。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)（**未落地**）。 |
| 包廂 Hub 引擎（go） | Booth Hub Engine／Hub 引擎 | 包廂**權威**；連 Platform（BoothAnchor **必須** WSS、門牌 mint、`device_token`）。Guest 握手經 Anchor（ENGINE §10.7）。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)。 |
| 包廂 Peer 引擎（go） | Booth Peer Engine／Peer 引擎 | headless **來源裝置**（監控鏡頭、邊緣掛檔）；經 Hub 本機 **`peerCap`** 進房；**不**連 Platform。外部 **`pg-boothd peer`**（私有 monorepo `pg-booth`）。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)。 |
| 包廂殼（go） | Booth Shell／包廂殼 | 包廂 **UI 客戶端**：只連 **Hub**、送 intent；**無** Hub 權威。含 go `/room`、遠端 **Operator** 面、`pg-booth-desktop` WebView Shell。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)。 |
| 包廂私有 monorepo（go） | `pg-booth` | **私有** Rust workspace；含 **`pg-boothd`**（headless CLI）、**`pg-booth-desktop`**（Tauri 輕量常駐）與共用 **`crates/booth-*`**；**非**開源；**不在** `playgrounds`。契約見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)、[PG-GO-ROOM-TAURI-PLAN.md](./PG-GO-ROOM-TAURI-PLAN.md)。 |
| 包廂常駐 daemon（go） | `pg-boothd` | **`pg-booth` monorepo** 內 headless CLI（hub／`peer`）；專業／NAS／邊緣常駐；瀏覽器 `/room` 為 Embedded Hub 替代。契約見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)。 |
| 桌面常駐包廂（go） | `pg-booth-desktop` | **`pg-booth` monorepo** 內 Tauri 安裝包；托盤 7×24、關窗不散場；一般會員輕量路徑；與 `pg-boothd` **共用 Hub 契約**與 Rust crates。見 [PG-GO-ROOM-TAURI-PLAN.md](./PG-GO-ROOM-TAURI-PLAN.md)。 |
| 包廂錨點（Platform） | BoothAnchor／包廂錨點 | Platform **`BoothAnchorDO`**：**Hub 必須**長連 WSS；中繼 Operator 遠端連回、**Guest join 握手**（`booth.join.*`；見 ENGINE §10.7）；**不**載媒體；**不**管 Peer 配對。Dash「連回包廂」。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md)。 |
| 包廂遠端操作者（go） | Operator／遠端操作者 | 帳號擁有者的 **owner 認證 Roster 節點**（`kind: operator`；Hub leaf）＋ **Remote Owner Shell** UI（go `/room/remote`）。與本地 `/room` 同 **Owner** 能力：導播 cast、**Hub 私有片庫讀寫**（Owner file channel）、開鏡頭／麥（`session_camera`／`session_mic`）、`session_play` 入座；**一條 WebRTC 連線**即可看＋說＋控。**進門**用 `operatorCap`（**不走** `/i/`）；拓樸上與 Guest **同為** Hub leaf（§6.2）。監控切台只是情境之一。見 [PG-GO-ROOM-ENGINE-PLAN.md](./PG-GO-ROOM-ENGINE-PLAN.md) §6、§6.1。 |
| Roster 節點（go） | Roster 節點／Hub leaf | 連上 Booth Hub 的客戶端（瀏覽器分頁、`pg-boothd peer` 行程）；各佔一 `peerId`、在 `members` 列出。`kind`＝`guest`｜`peer`｜`operator`（進門路徑標籤；**不是**能力上限）。權限由 owner／`director`／委任決定。見 ENGINE §6.2。 |
| 包廂 localhost／Agent harness（go） | 包廂 harness／`__goRoomDev` | **僅** `go:dev`＋loopback：注入 field API key（免 SSO）、可選 `dev_mint`／`dev_join`、多 tab 走真 `/room`＋`/i/`＋正式 Platform。**預設不開** `platform:dev`。**不是**產品面；**不是** `/dev/room-xfer`（傳檔 BC 隔離）。見 [PG-GO-ROOM-DEV-HARNESS-PLAN.md](./PG-GO-ROOM-DEV-HARNESS-PLAN.md)。 |
...
| Platform Invite（遊樂場） | Invite／`#pg=` | 一條邀請（短連結或深鏈）；**多人可經同一連結加入**。內嵌 intent；**不**預帶 WebRTC offer。每次加入＝短命 join。kind：**`invite.room`（連線唯一門）**、歷史 `invite.compose`（Superseded）、`signal.handshake` 等。**儲存：** **`InviteDurableObject`**（short 鏈、metadata、`join_cap`、TTL）。**鑄造：** SAM 或 go 殼經代理呼叫 Platform API（持殼頁**記憶體** API key）；**非**後台 UI。**消費者主路徑**＝go 短鏈；場 `#pg=` 可留相容。**臨時生命週期**（需網路／Host）；**不是**離線遊戲入口。見 DEC-047、[PG-PLATFORM-DASH-SPEC.md](./PG-PLATFORM-DASH-SPEC.md) §7、[PG-GO-CLIENT-PLAN.md](./PG-GO-CLIENT-PLAN.md)、[PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)。 |
...
| invite.compose（Platform） | 複合邀請（**Superseded**） | **連線遊戲已廢**；歷史 intent：開指定 SAM → 放大畫布 → 詢問入座。現行連線＝**`invite.room`** 包廂 → **`session_play`**。Invite DO **仍存**供舊鏈／metadata。見 DEC-047、[PG-GO-HOST-INVITE-PLAN.md](./PG-GO-HOST-INVITE-PLAN.md)。 |
| invite.room（Platform） | 包廂邀請 | **連線唯一門**：請人（或自己的另一台）進 **`play`／`go` Booth Hub** 包廂殼（進門無 SAM；開局＝`session_play`）；consent 後 Roster 握手 **經 BoothAnchor**（非 Invite DO long poll）。**Invite DO** 管 short 鏈、metadata、`join_cap`、TTL。主面 go＝`/room`；短鏈 `/i/`；**可多人 join**；TTL＝門牌。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、[ENGINE §10.7](./PG-GO-ROOM-ENGINE-PLAN.md)。 |
