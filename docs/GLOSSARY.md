...
| SAM 小品型錄 | 小品／`/sam/` | 開源 SAM 範本清單（遊戲、工具、代理…）；權威 URL＝場網 [`https://play.samkuo.me/sam/`](https://play.samkuo.me/sam/)（任意場＝同 origin 的 `/sam/`；與場殼同 Worker／PWA）。一鍵開＝同場 open 管線（`/?open=<source>` 或殼內）。資料權威＝[`catalog/entries/`](../catalog/entries/)（YAML；建置產 `samCatalog.generated.ts`＋`/catalog/v1.json`）。人機 UX＝搜尋／密度／精選貨架（Draft：[PG-CATALOG-UX-PLAN.md](./PG-CATALOG-UX-PLAN.md)）；**不**綁部落格散文 layout、**不**要求 Astro 靜態頁。部落格舊 `/sam/` 轉址至此。非 `/tools/`、非文件站。見 DEC-041／046／048、[PG-CATALOG-PLAN.md](./PG-CATALOG-PLAN.md)、[PG-CATALOG-QUERY-PLAN.md](./PG-CATALOG-QUERY-PLAN.md)。 |
...
| 純玩版客戶端（Playgrounds） | 純玩版／`go.samkuo.me` | 獨立 Worker＠**`https://go.samkuo.me`**：**無編輯環境**、不依賴持久 OPFS；受邀 Guest 掃碼／開短鏈，只參與**當下 Invite 指定的 SAM／session**（同 session 可多局，由該 SAM 決定）。UI **須**露出山姆鍋 logo 與遊樂場主網址 **`play.samkuo.me`**。**不是**場。見 [PG-GO-CLIENT-PLAN.md](./PG-GO-CLIENT-PLAN.md)、DEC-050。 |
| Platform Invite（遊樂場） | Invite／`#pg=` | 一條邀請（短連結或深鏈）；**多人可經同一連結加入**。內嵌 intent；**不**預帶 WebRTC offer。每次加入＝短命 join。kind 含 `signal.handshake`、`invite.compose`。**鑄造：** SAM 經場殼代理呼叫 Platform API（持殼頁**記憶體** API key）；**非**後台 UI。**消費者主路徑**＝go 短鏈；場 `#pg=` 可留相容。**不是** Platform 註冊邀請、**不是** `#roster=`、**不是** `#pg_provision=`。見 DEC-047、[PG-PLATFORM-DASH-SPEC.md](./PG-PLATFORM-DASH-SPEC.md) §7、[PG-GO-CLIENT-PLAN.md](./PG-GO-CLIENT-PLAN.md)。 |
...
| Platform 短連結 | `/i/<short_id>` | 對 **Invite** 穩定的短 URL。**Canonical**＝**`https://go.samkuo.me/i/<short_id>`**（邀請 QR 預設）；`api.samkuo.me/i/…` 可 302 至 go。與 Invite 同壽命；非通用縮址。見 DEC-047／050、[PG-GO-CLIENT-PLAN.md](./PG-GO-CLIENT-PLAN.md)。 |
