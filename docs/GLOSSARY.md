...
| 純玩版包廂（go） | 包廂／`/room` | go 殼層**臨時隔間**：壽命＝**主持 `/room` 畫面還開著**（關分頁／重整／結束＝散場）。**已登入會員開 `/room` 即包廂主面**（不必先請人；按「請人進來」才鑄門牌）。用 Invite 短鏈 **`/i/<short>`** 請人進來，**也可給自己的另一台掃**（再開 `/room`＝另一間空的，不是連上既有這一間）。**入座不鎖 1:1**；**在場視訊僅 1:1**（鏡頭只在兩人時）。短鏈 TTL 只管能不能請**新人**，**不是**包廂租期。Guest 同意後留在 `/i/`。產品三個動詞：**說**（文字）、**掛／存**（分享區目錄，點下載才串流寫入使用者選的檔）、**播出**（鏡頭僅兩人；本機影片／音樂＝節目，可多人收；走 RTP，與下載分開）。**檔 bytes 與音視訊 RTP 皆直連優先**（Guest↔Guest 由主持 DC 介紹 mesh；失敗才 Host 轉送；不經 Platform 第二輪）。資料面只走 WebRTC，**不**雲存、**不**經 Platform 中繼、**不錄製**。**不是**局內 overlay 對話、**不是** `invite.compose` 開 SAM、**不是**大廳公開聊天區、**不是**可離線／可收藏房號、**不是**視訊會議。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、DEC-050／045／047。 |
