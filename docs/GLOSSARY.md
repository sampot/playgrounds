...
| 總管範本（遊樂場） | 總管範本 | 開源總管角色 SAM 種子（[`sampot/pg-steward`](https://github.com/sampot/pg-steward)；含 BYOK 對話與 HOST 工具）。經 `/sam/` 或 `?open=sampot/pg-steward` 開入後再「設為總管」。遊樂場**不**內嵌；硬核使用者亦可自寫對口骨架。 |
