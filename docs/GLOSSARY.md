...
| 純玩版包廂（go） | 包廂／`/room` | go 殼層**臨時隔間**：**已登入會員開 `/room` 即包廂主面**（不必先請人）；用 Invite 短鏈 **`/i/<short>`** 請人進來，**同一門牌可多人**（不鎖 1:1）。資料面只走 WebRTC（文字；檔案＝分享區目錄，點下載才串流寫入使用者選的檔，**不**經 SW／OPFS、**不**整檔進 RAM；契約預留音視訊與桌機投放），**不**雲存、**不**經 Platform 中繼。**不是**局內 overlay 對話、**不是** `invite.compose` 開 SAM、**不是**大廳公開聊天區、**不是**可離線房間。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、DEC-050／045／047。 |
...
| invite.room（Platform） | 包廂邀請 | Invite intent：請人進 go **包廂**殼面（無 SAM）；consent 後 Roster 握手。開這一間的主面是 `/room`（進門即 UI）；短鏈仍 `/i/`；**可多人 join**。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)。 |
