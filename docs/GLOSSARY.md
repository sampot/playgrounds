...
| 純玩版遊樂場大廳 Lobby（go） | 大廳／lobby | go **首頁 `/`** 的 **室內遊樂場大廳** canvas 殼（湯姆熊式：走道、機台、服務台；**不是** outdoor 全園、**不是** RPG 道具店）：**單一 2D `<canvas>`**；**輕量行走**＋hit-test（老闆／布告欄／詢問處／**機台**／後場／**包廂門**等）；**殼是大廳、小品仍是小品**；清單模式（DOM）永遠可達；`/s/`／`/i/`／`/room` **深鏈 bypass**（包廂是獨立殼面：**靜態內景**＋電視，**不是**大廳可行走地圖、**不是** SAM 遊玩 canvas）。**禁止** DOM 當大廳地圖本體。見 [PG-GO-SHOP-LOBBY-PLAN.md](./PG-GO-SHOP-LOBBY-PLAN.md)、[PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、DEC-050。 |
| 純玩版包廂（go） | 包廂／`/room` | go 殼層**臨時隔間**：壽命＝**主持 `/room` 畫面還開著**。**已登入會員開 `/room` 即包廂主面**（按「請人進來」才鑄門牌）。短鏈 **`/i/<short>`** 請人進來，也可給自己的另一台掃。**入座不鎖 1:1**。**主面＝包廂電視**（節目 RTP；主持指定來源；電影＝來源端 `captureStream`）。每人自帶裝置可掛／下載／**私下播**（DC），**不跟電視互斥**。開口為主、文字為輔助抽屜。靜態內景延續大廳畫風（**不走動**）。分享目錄只掛檔。**現況經主持 Hub 轉送**（mesh 延後）。資料面只走 WebRTC，**不**雲存、**不錄製**。**不是**視訊會議格子牆、**不是**全頁聊天室。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、DEC-050／045／047。 |
...
| invite.room（Platform） | 包廂邀請 | Invite intent：請人（或自己的另一台）進 go **包廂**殼面（無 SAM）；consent 後 Roster 握手。開這一間的主面是 `/room`（進門即 UI；**按需鑄**這張邀請）；短鏈仍 `/i/`（Guest 留在此）；**可多人 join**；TTL＝門牌，過期不散包廂。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)。 |
