...
| 純玩版包廂（go） | 包廂／`/room` | go 殼層**臨時隔間**：已登入 Host 鑄 Invite，Guest 開 **`/i/<short>`** 進同一間；資料面只走 WebRTC（文字／檔案；契約預留音視訊與桌機投放），**不**雲存、**不**經 Platform 中繼。Host 主面 **`/room`**；**不是**局內 overlay 對話、**不是** `invite.compose` 開 SAM、**不是**大廳公開聊天區、**不是**可離線房間。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、DEC-050／045／047。 |
...
| invite.room（Platform） | 包廂邀請 | Invite intent：進 go **包廂**殼面（無 SAM）；consent 後 Roster 握手。Host 主面 `/room`；短鏈仍 `/i/`。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)。 |
