...
| 純玩版遊樂場大廳 Lobby（go） | 大廳／lobby | go **首頁 `/`** 的 **室內遊樂場大廳** canvas 殼（湯姆熊式：走道、機台、服務台；**不是** outdoor 全園、**不是** RPG 道具店）：**單一 2D `<canvas>`**；**輕量行走**＋hit-test（老闆／布告欄／詢問處／**機台**／後場等）；**殼是大廳、小品仍是小品**；清單模式（DOM）永遠可達；`/s/`／`/i/` **深鏈 bypass**。**禁止** DOM 當大廳地圖本體；**不是** SAM 遊玩 canvas。見 [PG-GO-SHOP-LOBBY-PLAN.md](./PG-GO-SHOP-LOBBY-PLAN.md)、DEC-050。 |
