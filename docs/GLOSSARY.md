...
| 純玩版包廂（go） | 包廂／`/room` | go 殼層**臨時隔間**：壽命＝**主持 `/room` 畫面還開著**。**已登入會員開 `/room` 即包廂主面**（按「請人進來」才鑄門牌）。短鏈 **`/i/<short>`** 請人進來，也可給自己的另一台掃。**入座不鎖 1:1**。產品動詞：**說**、**掛**（分享目錄＝檔，**不掛資料夾**）、**要**（下載／檢視／播放檔；收看 live 在在場名單）。**每人同時一條 live**（鏡頭或畫面分享，可含聲；WebRTC）。影音檔＝漸進下載＋本機播放器，不佔 live。**現況經主持 Hub 轉送**（mesh 延後）。資料面只走 WebRTC，**不**雲存、**不錄製**。**不是**視訊會議。見 [PG-GO-ROOM-PLAN.md](./PG-GO-ROOM-PLAN.md)、DEC-050／045／047。 |
