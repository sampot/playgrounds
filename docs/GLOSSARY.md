...
| 純玩版客戶端（Playgrounds） | 純玩版／`go.samkuo.me` | 獨立 Worker＠**`https://go.samkuo.me`**：**無編輯環境**、不依賴持久 OPFS 沙盒庫；**同時只跑一個 SAM**。啟動含（1）Invite 短鏈入座（**臨時**；**不能離線**）（2）型錄 **`/s/<catalog_id>`** 單機傳閱（3）首頁 `/` 至多 3 則**game**推薦（picks 優先）。`/s/` 當前為 **`kind: game`** 時可換片（下一個／推薦≤3，皆為其他 game），並可**加主畫面／造訪後離線／本機分數**（鍵＝`catalog_id`）；`/i/` 不換片、不離線。UI **須**露出山姆鍋 logo（→ **`play.samkuo.me/`**）、「山姆鍋遊樂場」文案（→ **`play.samkuo.me/sam/?kind=game`** 遊戲分類），以及條件允許時的 Header「分享」（開分享面→`/s/<id>`，**不是**邀請短鏈）。**不是**場。見 [PG-GO-CLIENT-PLAN.md](./PG-GO-CLIENT-PLAN.md) §5.5／§5.6／§6.4／§6.5、DEC-050。 |
| 型錄 SAM 傳閱連（go） | `/s/<catalog_id>`／go 分享網址 | 只認型錄穩定 **`id`** 的純玩深鏈：**`https://go.samkuo.me/s/<id>`**。go **建置內嵌** catalog 解析；無 Invite／join／TTL。型錄列「分享」與 go Header「分享」只出此形；皆開**頁內分享面**（系統分享／**QR**／複製；現場掃碼快樂路徑；型錄面網址可點開新分頁）；**分享 title／社群 `og:title`＝該筆 `entry.title`**（每 id 不同；首包 HTML 可見）。當前為 **game** 時可「下一個」／推薦≤3 換片（候選皆 game；非 game 不換片）；可安裝、造訪後離線再玩、本機分數。**不是** `/i/`、**不是**場 `?open=`／`view=canvas`、**不是**「一鍵開」、**不是** go 上完整型錄、**不是**邀請 QR。見 DEC-050、[PG-GO-CLIENT-PLAN.md](./PG-GO-CLIENT-PLAN.md) §5.5／§5.5.1／§5.6、[PG-CATALOG-UX-PLAN.md](./PG-CATALOG-UX-PLAN.md)。 |
| Platform Invite（遊樂場） | Invite／`#pg=` | 一條邀請（短連結或深鏈）；**多人可經同一連結加入**。內嵌 intent；**不**預帶 WebRTC offer。每次加入＝短命 join。kind 含 `signal.handshake`、`invite.compose`。**鑄造：** SAM 經場殼代理呼叫 Platform API（持殼頁**記憶體** API key）；**非**後台 UI。**消費者主路徑**＝go 短鏈；場 `#pg=` 可留相容。**臨時生命週期**（需網路／Host）；**不是**離線遊戲入口。**不是** Platform 註冊邀請、**不是** `#roster=`、**不是** `#pg_provision=`、**不是**型錄 `/s/`。見 DEC-047、[PG-PLATFORM-DASH-SPEC.md](./PG-PLATFORM-DASH-SPEC.md) §7、[PG-GO-CLIENT-PLAN.md](./PG-GO-CLIENT-PLAN.md)。 |
