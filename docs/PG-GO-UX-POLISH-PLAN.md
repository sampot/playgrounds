...
- 新增明確 phase（建議 `cancelled`）或同等 UI 分支：文案「已取消」、主 CTA「回遊樂場大廳」、次要「重新開啟此邀請」（重整或再 resolve）。
- 禁 `alert`。
...
- [x] `/i/` 取消 → 明確已取消＋回遊樂場大廳；不再像載入中（`phase: cancelled`；手測待）
...
| 用 | 不用 |
| --- | --- |
| 已取消、回遊樂場大廳、搜尋遊戲名稱 | 正在讀取（取消後）、只能輸入 id、邀請中分享（灰鈕） |
