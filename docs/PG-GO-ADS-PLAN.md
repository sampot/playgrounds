...
| **1. House 版位** | `GoAdSlot`（320×100／寬 728×90）＋house；`/` ＋ `/s/` 載入＋**`/apps` 列表下方**；boot 後收起 | 點進其他 `/s/<id>`；明示廣告且與內容卡區隔；非迷你型錄；`/help`／`/i/`／玩中無 | **已落地**（含 `/apps`） |
...
其餘已定：投放＝`/` ＋ `/s/` 載入＋**`/apps` 列表下方**；不掛 `/help`／`/i/`／玩中；Phase 1＝house 自推站內 game。
