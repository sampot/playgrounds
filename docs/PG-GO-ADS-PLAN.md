...
| **`/room` 包廂主面** | ✅ | **浮在主視訊區內**（同一 `GoAdSlot`）。**節目串流（片子／live／日後開局）自動藏。** 不佔流高度、不另掛 sticky 搶電視。內景後牆看板若還畫＝裝飾、不進熱點。**live／開局時點擊須頁內確認**（開局中先結束這一局或一併說明散場）再結束／離開並進 `/s/` |
...
| 2026-08-19 | **修訂：** 包廂廣告：**劇院態藏**；廳態直式可掛；短橫屏／頂列收起不搶電視 |
