...
- **UX：** mobile-first；**IAB 形廣告尺寸槽**、無 CLS；**`/s/` 載入中為一級投放**；**`/apps` 列表下方一格**；玩中與 Invite／`/help` 不露。
...
| **`/apps`** | ✅ | **列表下方**一格（§5.1.1）；空態／載入中不掛 |
| **`/room` 包廂主面** | ✅ | **浮在主視訊區內**（同一 `GoAdSlot`）。**節目串流（片子／live／日後開局）自動藏。** 不佔流高度、不另掛 sticky 搶大螢幕。內景後牆看板若還畫＝裝飾、不進熱點。**live／開局時點擊須頁內確認**（開局中先結束這一局或一併說明散場）再結束／離開並進 `/s/` |
| **`/help`** | ❌ | 披露文案可寫在此，但**無**廣告槽 |
| **`/s/<id>` 錯誤／尚無畫布** | ✅ 可同槽 | 載入失敗頁可保留同一槽；**不**在錯誤上強制多一層 |
| **`canvasActive` 遊玩中** | ❌ 預設關 | boot 成功 → **立刻收起／卸載**版位；勿搶觸控 |
| **`/i/` 遊戲 Invite** | ❌ | 對弈／同意面／其 SAM 下載等待**不**掛 |
| **`/i/` 包廂 Guest 主面** | ✅ | 同意進包廂後與主持同一 `GoRoomSurface`（網址仍 `/i/`）；**不是**遊戲 Invite |
...
| 項 | 規格 |
| --- | --- |
| **何時** | 僅有離線列表（≥1）時；loading／error／空態**不**掛 |
| **分頁** | 每頁最多 **5** 款（`APPS_PAGE_SIZE`）；`?page=` 可選；分頁控制在**列表上方** |
| **位置** | **當頁**列表**下方**一格（列表與進階區之間） |
...
9. **視覺區隔（硬）：** 使用專屬廣告 surface／border，勿沿用一般內容卡片的完整配色；同時以標籤、框體與留白區隔，**不可只靠顏色**。`/apps` 列表下方版位在窄屏與亮／暗主題下都須與離線遊戲列清楚可辨。
...
- 掛點：`/` 頁底槽；`/s/` 載入 UI；**`/apps` 列表下方**；`canvasActive` → 卸載。
...
| **1. House 版位** | `GoAdSlot`（320×100／寬 728×90）＋house；`/` ＋ `/s/` 載入＋**`/apps` 列表下方**＋**包廂 `/room`（含 Guest 進門後主面）**；boot 後收起 | 點進其他 `/s/<id>`；明示廣告且與內容卡區隔；非迷你型錄；`/help`／遊戲 `/i/`／玩中無 | **已落地**（含 `/apps`、包廂） |
...
- [x] 遊戲 `/i/`、`/help` 與遊玩中無版位；**`/apps` 列表下方有一格**；**包廂主面有一格**（live 點擊須確認）
...
其餘已定：投放＝`/` ＋ `/s/` 載入＋**`/apps` 列表下方**＋**包廂主面**；不掛 `/help`／遊戲 `/i/`／玩中；Phase 1＝house 自推站內 game。
...
| 2026-08-17 | **修訂：** `/apps` 改**列表下方**一格；每頁 **5** 款；分頁控制在列表上方 |
| 2026-08-19 | **修訂：** 包廂主面掛同一 `GoAdSlot`（主持 `/room` 與 Guest 同意後的包廂 UI）；遊戲 Invite `/i/` 仍不掛；內景後牆看板裝飾；live 點擊頁內確認後才進 `/s/` |
| 2026-08-19 | **修訂：** 包廂廣告：**劇院態藏**；廳態直式可掛；短橫屏／頂列收起不搶大螢幕 |
| 2026-08-19 | **修訂：** 包廂廣告改浮在主視訊區；串流自動藏（短橫屏不再另外藏） |
